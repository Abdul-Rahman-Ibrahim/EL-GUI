# -*- coding: utf-8 -*-
"""
Created on Sat Nov 26 14:17:27 2022
...
class GUI:
    ...
    def __init__(self):
        ...
        try:
            ...
            for line in db:
                ...
                id, lastname, firstname, gender, contact = line[-2], line[0], line[1], line[2], line[5]
    ...
    def AddNewWindow(self):
        ...
        lbl_rmks = Label(ContactForm, text="Remark", font=('arial', 14), bd=5)
        lbl_rmks.grid(row=9, sticky=W)
        ...
        rmks = Entry(ContactForm, textvariable=self.RMKS,  font=('arial', 14))
        rmks.grid(row=9, column=1)
        

        #==================BUTTONS==============================
        btn_addcon = Button(ContactForm, text="Save", width=50, command=self.SubmitData)
        btn_addcon.grid(row=10, columnspan=2, pady=10)
    ...
    def OnSelected(self, event):
        ...
        try:
            file = open('DB.csv')
            for line in file:
                if line.strip().split(',')[-2] == str(mem_id):
                    line = line.strip().split(',')
                    lastname, firstname, gender, age, address, contact, transaction, amount, status, id, rmks = line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7], line[8], line[-2], line[-1]
                    self.AGE.set(age)
        ...
        if ask:
            ...
            self.Customertree = ttk.Treeview(self.CustomerTableMargin, columns=("MemberID", "Date", "Time", "Address", "Age", "Transaction", "Amount", "Status", "Remark"), height=400, selectmode="extended", yscrollcommand=self.scrollbary.set, xscrollcommand=self.scrollbarx.set)
            ...
            self.Customertree.heading('Remark', text="Remark", anchor=W)
            ...
            self.Customertree.column('#8', stretch=NO, minwidth=0, width=90)
            ...
            for line in content:
                line = line.strip().split(',')
                id, (date, time), address, age, transaction, amount, status, rmks = line[-2], line[0].split(), line[1], line[2], line[3], line[4], line[5], line[-1]
                tup = (id, date, time, address, age, transaction, amount, status, rmks)
    ...
    def SubmitData(self):
        if  self.FIRSTNAME.get() == "" or self.LASTNAME.get() == "":
            ...
            file.write(self.LASTNAME.get().title() + "," + self.FIRSTNAME.get().title() + "," + self.GENDER.get() + "," + self.AGE.get() + "," + self.ADDRESS.get() + "," + self.CONTACT.get() + "," + self.TRANSACTION.get() + "," + self.AMOUNT.get() + "," + self.STATUS.get() + "," + str(id) + ","  + self.RMKS.get()+ "\n")
            file.close()
            
            file = open('Customers/'+str(id)+'.csv', 'a')
            now = datetime.now()
            dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
            file.write(dt_string + "," + self.ADDRESS.get() + "," + self.AGE.get() + "," + self.TRANSACTION.get()+ "," + self.AMOUNT.get() +  "," + self.STATUS.get() + "," + str(id) + "," + self.RMKS.get() + "\n")
    ...
    def UpdateData(self):
        ...
        for line in file:
            if line.strip().split(',')[-2] == str(mem_id):
                tmpfile.write(self.LASTNAME.get().title() + "," + self.FIRSTNAME.get().title() + "," + self.GENDER.get() + "," + self.AGE.get() + "," + self.ADDRESS.get() + "," + self.CONTACT.get() + "," + self.TRANSACTION.get() + "," + self.AMOUNT.get() + "," + self.STATUS.get() + "," + str(mem_id) + "," +self.RMKS.get()+"\n")
            else:
                tmpfile.write(line)
